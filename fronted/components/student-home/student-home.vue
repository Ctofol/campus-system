  ...
  <view class="home-container">
    ...
    <view class="content-wrapper" :style="{paddingTop: (statusBarHeight + 44) + 'px'}">
      ...
      <view class="header-section">
        <!-- 任务轮播通知 (Swiper) -->
        <view class="tasks-swiper-container" v-if="teacherTasks.length > 0">
           <swiper class="task-swiper" :vertical="true" :autoplay="true" :interval="4000" :duration="500" :circular="true">
             <swiper-item v-for="(task, index) in teacherTasks" :key="task.id">
               <view class="teacher-task-box" @click="handleTaskClick(task)">
        ...
        <view class="student-func-grid">
          ...
          <view class="stu-func-item" @click="uni.switchTab({url: '/pages/tab/stats'})">
            <view class="stu-func-icon">📊</view><text class="stu-func-name">体测成绩</text>
          </view>
          <view class="stu-func-item" @click="uni.switchTab({url: '/pages/tab/mine'})">
    ...
    <!-- 底部占位 -->
    <!-- Native TabBar handles spacing, but a small padding is good -->
...
// import { onShow, onLoad } from '@dcloudio/uni-app'; // Removed, handled by parent
...
const startTestProject = (item) => { 
    // Use switchTab for stats tab
    uni.switchTab({url: '/pages/tab/stats'}); 
};
const startTraining = (item) => { 
  // 实战训练跳转
  // Note: /pages/run/run is now a tab. Passing query params to a tab page is tricky in Uni-app (switchTab ignores query).
  // We might need to save parameters to global store/storage or use a non-tab run page for training details.
  // For now, let's assume we navigate to the run tab.
  uni.switchTab({
    url: `/pages/tab/run`
  }); 
  // TODO: Handle training mode params in run tab via storage or event bus
};
