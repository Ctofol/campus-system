...
<script setup>
import { ref } from 'vue'
import { importStudents, importTeachers, downloadStudentTemplate, downloadTeacherTemplate } from '../api/index.js'
...
const handleImport = async (file, type) => {
  ...
  try {
    const fn = type === 'student' ? importStudents : importTeachers
...
const downloadTpl = (type) => {
  ...
  const url = type === 'student' ? downloadStudentTemplate() : downloadTeacherTemplate()
